...
if( $t_show_reporter || $t_show_handler || $t_show_due_date ) {
	...
	if( $t_show_reporter ) {
		...
		if( ON == config_get( 'limit_reporters' )
			&&  !access_has_project_level( access_threshold_min_level( config_get( 'limit_reporters_override_threshold', null, null, $t_bug->project_id ) ) + 1, $t_bug->project_id )
		) {
...
if( $t_show_platform || $t_show_os || $t_show_os_version ) {
	...
	if( $t_show_platform ) {
		...
		if( config_get( 'allow_freetext_in_profile_fields' ) == OFF ) {
			...
			echo '<input type="text" id="platform" name="platform" class="typeahead input-sm" autocomplete = "off" size="50" maxlength="50" tabindex="' . helper_get_tab_index_value() . '" value="' . string_attribute( $t_bug->platform ) . '" />';
		}
